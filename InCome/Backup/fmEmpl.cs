	...
	public class fmEmpl : System.Windows.Forms.Form
	{
		...
		public int addToQueue(int id,string code,bool add)
		{
			...
			try
			{
				...
				if(add)
				{
					...
					sqlCom.Parameters.Add(new OleDbParameter("@codeprint",CEncode128.Encode128(code)));
					sqlCom.Parameters.Add(new OleDbParameter("@id",id));					
				}
		...
		public bool delQueue()
		{
			try
			{
				OleDbConnection sqlCon=new OleDbConnection(conStr);
				sqlCon.Open();				
				OleDbCommand sqlCom=new OleDbCommand("delete from CodeToPrint",sqlCon);
				sqlCom.CommandTimeout=60;
		...
		private bool delUserByID(int id)
		{
			try
			{
				OleDbConnection sqlCon=new OleDbConnection(conStr);
				sqlCon.Open();
				OleDbCommand sqlCom=new OleDbCommand("delete from EmplMain where MID="+id.ToString(),sqlCon);
				sqlCom.CommandTimeout=60;
		...
		{		
			...
			try
			{
				OleDbConnection sqlCon=new OleDbConnection(conStr);
				sqlCon.Open();
				OleDbCommand sqlCom=new OleDbCommand(sql);
				sqlCom.CommandTimeout=60;
				sqlCom.Connection=sqlCon;
		...
		private bool changeUserByID(int id)
		{
			...
			OleDbConnection sqlCon=new OleDbConnection(conStr);
			sql="update EmplMain set MCODE=?,MLOGIN=?,MPASSWORD=?,MLOCKED=?,MUSEPASSW=?,MDEPARTMENT=?,MGROUP=?,MSURNAME=?,MNAME=?,MSECNAME=?, ";
			sql+="MBIRTHDAY=?,MDOCUMENT=?,MDOCTYPE=?,MMALE=?,MEMPDATE=?,MPHONE=?,MMOBPHONE=?,MEMAIL=?,MCOUNTRY=?,MREGION=?,MDISTRICT=?,MSETTLEMENT=?,MADDRESS=?,MPHOTO=? where mid=?";
			OleDbCommand sqlCom=new OleDbCommand(sql,sqlCon);
			sqlCom.CommandTimeout=60;
			sqlCom.Parameters.Add(new OleDbParameter("@MCODE",tbCode.Text));
			sqlCom.Parameters.Add(new OleDbParameter("@MLOGIN",tbLogin.Text));
			sqlCom.Parameters.Add(new OleDbParameter("@MPASSWORD",tbPassword.Text));
			sqlCom.Parameters.Add(new OleDbParameter("@MLOCKED",(cbLocked.Checked)?"YES":"NO"));
			sqlCom.Parameters.Add(new OleDbParameter("@MUSEPASSW",(cbUsePassword.Checked)?"YES":"NO"));
			sqlCom.Parameters.Add(new OleDbParameter("@MDEPARTMENT",cbDepart.Text));
			sqlCom.Parameters.Add(new OleDbParameter("@MGROUP",cbGroup.Text));
			sqlCom.Parameters.Add(new OleDbParameter("@MSURNAME",tbSurname.Text));
			sqlCom.Parameters.Add(new OleDbParameter("@MNAME",tbName.Text));
			sqlCom.Parameters.Add(new OleDbParameter("@MSECNAME",tbSecname.Text));
			sqlCom.Parameters.Add(new OleDbParameter("@MBIRTHDAY",dtpBirth.Value));
			sqlCom.Parameters.Add(new OleDbParameter("@MDOCUMENT",tbDoc.Text));
			sqlCom.Parameters.Add(new OleDbParameter("@MDOCTYPE",tbDocType.Text));
			sqlCom.Parameters.Add(new OleDbParameter("@MMALE",cbMale.Text));
			sqlCom.Parameters.Add(new OleDbParameter("@MEMPDATE",dtpEmplDate.Value));
			sqlCom.Parameters.Add(new OleDbParameter("@MPHONE",tbPhone.Text));
			sqlCom.Parameters.Add(new OleDbParameter("@MMOBPHONE",tbMobphone.Text));
			sqlCom.Parameters.Add(new OleDbParameter("@MEMAIL",tbEmail.Text));
			sqlCom.Parameters.Add(new OleDbParameter("@MCOUNTRY",tbCountry.Text));
			sqlCom.Parameters.Add(new OleDbParameter("@MREGION",tbRegion.Text));
			sqlCom.Parameters.Add(new OleDbParameter("@MDISTRICT",tbDistrict.Text));
			sqlCom.Parameters.Add(new OleDbParameter("@MSETTLEMENT",tbSettlement.Text));
			sqlCom.Parameters.Add(new OleDbParameter("@MADDRESS",tbAddress.Text));
			...
			using(MemoryStream ms=new MemoryStream())
			{
				if(pbPhoto.Image!=null)
				{
					...
					if((bm=(pbPhoto.Image as Bitmap))!=null)
					{
						...
						sqlCom.Parameters.Add("@MPHOTO", OleDbType.Binary,photo.Length).Value=photo;
					}
				}
			}
			//...			
			sqlCom.Parameters.Add(new OleDbParameter("@mid",id));
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(fmEmpl));
			...
			this.groupBox3.Text = "Служебная информация";
			// 
			// label18
			// 
			...
			this.label18.Text = "Дата трудоустройства";
			// 
			// dtpEmplDate
			// 
			...
			this.cbLocked.Text = "Заблокирован";
			// 
			// cbUsePassword
			// 
			...
			this.cbUsePassword.Text = "Защитить паролем";
			...
			this.btnNewGroup.Font = new System.Drawing.Font("Microsoft Sans Serif", 12F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((System.Byte)(204)));
			...
			this.btnNewDepart.Font = new System.Drawing.Font("Microsoft Sans Serif", 12F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((System.Byte)(204)));
			...
			this.dtpBirth.Value = new System.DateTime(2007, 3, 16, 14, 16, 23, 477);
			// 
			// tbSecname
			// 
			this.tbSecname.Location = new System.Drawing.Point(70, 64);
			this.tbSecname.Name = "tbSecname";
			this.tbSecname.Size = new System.Drawing.Size(226, 20);
			this.tbSecname.TabIndex = 3;
			this.tbSecname.Text = "";
			// 
			// tbName
			// 
			this.tbName.Location = new System.Drawing.Point(70, 39);
			...
			// tbSurname
			// 
			this.tbSurname.Location = new System.Drawing.Point(70, 15);
			this.tbSurname.Name = "tbSurname";
			this.tbSurname.Size = new System.Drawing.Size(226, 20);
			...
			this.label6.Text = "Документ, подтверждающий личность";
			// 
			// label5
			// 
			...
			this.tvEmpl.SelectedImageIndex = -1;
		...
		private void btnPrintQueue_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
					OleDbDataAdapter sqlDA=new OleDbDataAdapter("SELECT FIO,CODE,DEPARTMENT,GRUPA,CODEPRINT,PHOTO FROM CodeToPrint",conStr);
				    sqlDA.SelectCommand.CommandTimeout=60;
					sqlDS = new DataSet();
					sqlDA.Fill(sqlDS);					

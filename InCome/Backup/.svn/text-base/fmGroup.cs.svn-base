...
namespace InCome
{
	...
	public class fmGroup : System.Windows.Forms.Form
	{
		...
		public DataSet fillGroups()
		{
			...
			try
			{
				OleDbDataAdapter sqlDA=new OleDbDataAdapter("select GRID as 'Идентификатор',GRNAME as 'Название' from EmplGroup order by GRNAME",conStr);
		...
		private bool addGroup(string name)
		{
			try
			{
				...
				sqlCon.Open();
				OleDbCommand sqlCom=new OleDbCommand("insert into EmplGroup (GRNAME) values ('"+name+"')",sqlCon);
		...
		private bool changeGroup(int grid,string name)
		{
			try
			{
				...
				OleDbConnection sqlCon=new OleDbConnection(conStr);
				sqlCon.Open();
				OleDbCommand sqlCom=new OleDbCommand("update EmplGroup set GRNAME='"+name+"' where GRID="+grid.ToString(),sqlCon);
				sqlCom.CommandTimeout=60;
		...
		private bool delGroup(int grid)
		{
			try
			{
				OleDbConnection sqlCon=new OleDbConnection(conStr);
				sqlCon.Open();
				OleDbCommand sqlCom=new OleDbCommand("delete from EmplGroup where GRID="+grid.ToString(),sqlCon);
				if(sqlCom.ExecuteNonQuery()==0)
					return false;
				sqlCon.Close();
			}
		...
		public  static string [][] getGroups(string conStr)
		{
			...
			try
			{
				OleDbDataAdapter sqlDA=new OleDbDataAdapter("select GRNAME,GRID from EmplGroup order by GRNAME",conStr);
				sqlDA.SelectCommand.CommandTimeout=60;
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(fmGroup));
